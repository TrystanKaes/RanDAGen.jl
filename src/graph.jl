...
size(graph::Graph) = length(graph.vertices)
...
function add_edge!(graph::Graph, edge::Edge) begin
    if haskey(graph.vertices, edge.src) && haskey(graph.vertices, edge.dest)
...
function add_node!(graph::Graph, vertex::Vertex) begin
    if !haskey(graph.vertices, vertex.ID)
